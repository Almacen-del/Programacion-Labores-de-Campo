...
const modules=[
  ...
  {id:'gantt',title:'Gantt por lote y labor',label:'Gantt',glyph:'▤',stage:4},
  {id:'records',title:'Registros del maestro',label:'Registros',glyph:'◫'},
  {id:'lots',title:'Detalle por lote',label:'Lotes',glyph:'⌂',stage:4},
  {id:'indicators',title:'Indicadores',label:'Indicadores',glyph:'◔',stage:4},
...
export function Workspace({initial,brand,onLogout,onDenied}:{initial:Bootstrap;brand:ReactNode;onLogout:()=>void;onDenied:Denied}){
  ...
  const refresh=useCallback(async()=>{
    ...
    inFlight.current=true;setRefreshing(true);const request=new AbortController();controller.current=request;
  ...
  const stale=!master.lastCheckedAt || Date.now()-Date.parse(master.lastCheckedAt)>15*60*1000;
