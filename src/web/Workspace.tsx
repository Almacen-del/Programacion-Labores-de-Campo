...
const modules=[
  ...
  {id:'inventory',title:'Inventario general',label:'Inventario',glyph:'▣'},
  {id:'alerts',title:'Alertas de datos',label:'Alertas',glyph:'△'},
  {id:'reports',title:'Reportes y respaldos',label:'Reportes',glyph:'▧'},
...
function Overview({master,onNavigate}:{master:Master;onNavigate:(id:ModuleId)=>void}){return <>
  ...
  <p className="notice">Solo se cuentan filas con lote y año de siembra. Si completas ambos en el maestro, la fila se incorpora en la siguiente revisión automática de 5 minutos.</p>
  <section className="metrics" aria-label="Resumen de la versión vigente"><Metric label="Registros del maestro" value={master.summary?.total} detail="Con lote y año de siembra"/><Metric label="Datos válidos" value={master.summary?.valid} detail="Sin hallazgos de validación" tone="good"/><Metric label="Con observaciones" value={master.summary?.observed} detail="Requieren revisión" tone="warning"/><Metric label="Bloqueados" value={master.summary?.blocked} detail="Excluidos de futuros cálculos" tone="danger"/></section>
...
export function Workspace({initial,brand,onLogout,onDenied}:{initial:Bootstrap;brand:ReactNode;onLogout:()=>void;onDenied:Denied}){
  ...
  else if(moduleId==='inventory')content=<Inventory master={master} onDenied={onDenied} onLot={lot=>{setBoard({...board,filters:{...board.filters,lot}});setModule('gantt')}}/>;
  else if(moduleId==='records')content=<Records master={master} onDenied={onDenied} onDetail={setDetail}/>;
  else if(moduleId==='reports')content=<Reports master={master} state={board} onChange={setBoard} onDenied={onDenied} onGantt={()=>setModule('gantt')}/>;
